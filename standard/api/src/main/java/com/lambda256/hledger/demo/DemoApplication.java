...
public class DemoApplication {
	...
	private static String aValue = "funding"; //진단서 변수 이름
	private static String bValue = "like"; //의무기록 변수 이름
	...
	private int fundingCount = 0; //진료 업데이트 개수
	private int likeCount = 0; //의무기록 업데이트 개수

	// private String org = "msp"; // 로그인 회원 조직
	// private static String pem_cert = "./cert.pem"; // 로그인 cert
	// private static String pem_key = "./key.pem"; // 로그인 key

	// private static String org = "doctor"; // 로그인 회원 조직
	// private static String pem_cert = "./cert-d.pem"; // 로그인 cert
	// private static String pem_key = "./key-d.pem"; // 로그인 key

	// private static String org = "mediteam"; // 로그인 회원 조직
	// private static String pem_cert = "./cert-m.pem"; // 로그인 cert
	// private static String pem_key = "./key-m.pem"; // 로그인 key

	private String org = "patient"; // 로그인 회원 조직
	private static String pem_cert = "./cert-p.pem"; // 로그인 cert
	private static String pem_key = "./key-p.pem"; // 로그인 key

	public static void main(String[] args) {
		//mvn spring-boot:run -Dspring-boot.run.arguments=./MSP접속파일.json,channel이름,체인코드이름,버전
		//위 명령어로 매개변수를 받음
	...
	public ResponseEntity<Object> getHistories(@RequestParam(value = "next", defaultValue = "0") String next) {
		...
		response.put("fundingCount", fundingCount); // 진단서 업데이트 수
		response.put("likeCount", likeCount); // 의무기록 업데이터 수
		return new ResponseEntity<>(response, HttpStatus.OK); //페이지에 숫자 전달
	}
	...
	public ResponseEntity<String> doTransaction(@RequestParam(value = "from", defaultValue = "World") String from,
		...
		try {
			...
			if(action.equals("input")){ // 진단서 업데이트
				executeChaincode(client, channel, aValue, "1");
			}if(action.equals("add")){ // 의무기록 업데이트
				executeChaincode(client, channel, bValue, "1");
			}
		} catch (Exception e) {
		...
		// 업데이트 완료 후 업데이트 수를 1씩 올림
		if (action.equals("funding") || action.equals("input")) fundingCount++; 
		if (action.equals("like") || action.equals("add")) likeCount++; 
	...
	private static void initialize() {
		// 접속 정보가 있는 파일 위치 (시작할 때 매개변수로 받아서 저장)
			...
			IOException, NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException
	{
		...
		String certificate = new String(IOUtils.toByteArray(new FileInputStream(new File(pem_cert))), "UTF-8");
		File privatekeyfile = new File(pem_key);
